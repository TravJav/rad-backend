...
class ContextManager:
    ...
    def __init__(self) -> None:
        ...
        self._permissions = {
            "register": [
                "radpair.src.health.api.app",
                "radpair.src.health.api.services.helpers.project_util",
                "radpair.tests.unit.test_context_manager",
            ],
            "system": {
                subprocess: [
                    "radpair.src.health.api.app",
    ...
    def _configure_env(self) -> None:
        ...
        if runtime_env not in self.ALLOWED_STATES:
            logging.critical("Unexpected data migration enviornment state. Exiting..")
    ...
    def register(self) -> None:
        ...
        logging.info("Created first-time environment registration hash from %s", self.caller_id)
    ...
    def generate_unique_uuid(self) -> str:
        """
        generates a unique string hash used by various pieces in the application
        Returns:
            str: hashed string requested by utility class
        """
        rnd = random.Random()
